...
```bash
./maintui -m models/7B/ggml-model.bin --prompt "Once upon a time"
```

#### Windows:

```powershell
maintui.exe -m models\7B\ggml-model.bin --prompt "Once upon a time"
```
...
```bash
./maintui -m models/7B/ggml-model.bin -n -1 --color -r "User:" --in-prefix " " -i -p \
'User: Hi
...
```powershell
maintui.exe -m models\7B\ggml-model.bin -n -1 --color -r "User:" --in-prefix " " -i -e -p "User: Hi\nAI: Hello. I am an AI chatbot. Would you like to talk?\nUser: Sure!\nAI: What would you like to talk about?\nUser:"
```
...
```bash
./maintui -m models/7B/ggml-model.bin --ignore-eos -n -1 --random-prompt
```

#### Windows:

```powershell
maintui.exe -m models\7B\ggml-model.bin --ignore-eos -n -1 --random-prompt
```

## Common Options

In this section, we cover the most commonly used options for running the `maintui` program with the LLaMA models:
...
The `maintui` program provides several ways to interact with the LLaMA models using input prompts:
...
The `maintui` program offers a seamless way to interact with LLaMA models, allowing users to engage in real-time conversations or provide instructions for specific tasks. The interactive mode can be triggered using various options, including `--interactive`, `--interactive-first`, and `--instruct`.
...
```sh
./maintui -r "User:" --in-prefix " " --in-suffix "Assistant:"
```
